...
local function addPlayers()
  rednet.open("left")
  while true do
    local _, newplayer = rednet.receive("holoplayer")
    players[#players + 1] = newplayer
  end
end
...
local function drawPlayers()
  for i=1, #players do
    local ox, oy, oz, name
    ox = players[i][1] - turtledata.x
    oy = players[i][2] - turtledata.y
    oz = players[i][3] - turtledata.z
    name = players[i][4]
    if ox < 6 and ox > -6 and oz < 6 and oz > -6 then
      if oy - yview == 0 or oy - yview == -1 then
        term.setCursorPos(ox + 7,oz + 7)
        term.setBackgroundColor(colors.gray)
        term.setTextColor(getColorFromName(name))
        if name == "diebeck" then term.setTextColor(colors.white) end

        if math.fmod(os.clock(),2) < 1 then
          term.write(string.sub(name,1,1))
        else
          term.write("\2")
        end
  ...
  while true do
    ...
    for ox=-5, 5 do
      for oz=-5, 5 do
        ...
        local commonblocks = {
          grass_block = colors.green,
          sand = colors.yellow,
          gravel = colors.lightGray,
          dirt = colors.brown,
          stone = colors.lightGray,
          cobblestone = colors.lightGray,
          andesite = colors.lightGray,
          diorite = colors.lightGray,
          granite = colors.lightGray,
          deepslate = colors.lightGray,

          water = colors.blue,
          lava = colors.lava,

          chest = colors.brown,
          furnace = colors.lightGray,

          coal_ore = colors.lightGray,
          deepslate_coal_ore = colors.lightGray,
          iron_ore = colors.lightGray,
          deepslate_iron_ore = colors.lightGray,
          gold_ore = colors.lightGray,
          deepslate_gold_ore = colors.lightGray,
          redstone_ore = colors.lightGray,
          deepslate_redstone_ore = colors.lightGray,
          diamond_ore = colors.lightGray,
          deepslate_diamond_ore = colors.lightGray
        }
        ...
        local commonsymbolcolor = {
          dirt = colors.lightGray,
          stone = colors.gray,
          deepslate = colors.black,
          diorite = colors.white,
          andesite = colors.cyan,
          granite = colors.orange,

          coal_ore = colors.black,
          deepslate_coal_ore = colors.black,
          iron_ore = colors.white,
          deepslate_iron_ore = colors.white,
          gold_ore = colors.yellow,
          deepslate_gold_ore = colors.yellow,
          redstone_ore = colors.red,
          deepslate_redstone_ore = colors.red,
          diamond_ore = colors.lightBlue,
          deepslate_diamond_ore = colors.lightBlue
        }

        --background color
        if commonblocks[gotblock] ~= nil then
          term.setBackgroundColor(commonblocks[gotblock])
        else
          term.setBackgroundColor(getColorFromName(gotblock))
        end

        --symbol color
        if commonsymbolcolor[gotblock] ~= nil then
          term.setTextColor(commonsymbolcolor[gotblock])
        else
          term.setTextColor(colors.white)
        end
        ...
        --symbol
        if gotblock then
          if commonsymbols[gotblock] ~= nil then
            term.write(commonsymbols[gotblock])
          else
...
parallel.waitForAll(save, drawData, drawBlocks, checkClicks, checkKeys, coolName, addPlayers, debug)